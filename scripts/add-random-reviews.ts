...
function generateRandomRoomNumber(): string {
...
async function addRandomReviews(formId: string, count: number = 20) {
  try {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (form.predefinedQuestions?.hasCustomRating && form.predefinedQuestions.customRatingItems) {
        ...
        form.predefinedQuestions.customRatingItems.forEach(item => {
          ...
          customRatings[`custom-rating-${item.id}`] = rating;
          overallRating += rating;
          ratingCount++;
        });
        Object.assign(answers, customRatings);
      }
      ...
      const predefinedAnswers: Record<string, any> = {};
      if (answers['rate-us']) {
